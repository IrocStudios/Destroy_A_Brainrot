...
function ShoulderCamera:setEnabled(enabled)
	...
	if self.enabled then
		...
		-- Sprint keybind removed — SpeedController.client.lua now owns WalkSpeed + sprint
...
function ShoulderCamera:onRenderStep(dt)
	...
	-- Communicate ADS state to SpeedController via IsADS BoolValue on character
	if self.currentRootPart and self.currentRootPart.Parent then
		local adsVal = self.currentRootPart.Parent:FindFirstChild("IsADS")
		if adsVal then
			adsVal.Value = self.zoomState
		end
	end
