...
function WeaponsGui.new(weaponsSystem)
	...
	coroutine.wrap(function()
		...
		-- UIScale drives crosshair zoom/weapon scaling instead of overwriting Size
		self.crosshairUIScale = self.crosshairFrame:FindFirstChildOfClass("UIScale")
		if not self.crosshairUIScale then
			self.crosshairUIScale = Instance.new("UIScale")
			self.crosshairUIScale.Parent = self.crosshairFrame
		end
		self.crosshairUIScale.Scale = 1

		local hitMarkerFrame = self.scalingElementsFolder:WaitForChild("HitMarker")
		self.hitMarker = hitMarkerFrame:WaitForChild("HitMarkerImage")

		-- UIScale drives hitmarker pop effect
		self.hitMarkerUIScale = hitMarkerFrame:FindFirstChildOfClass("UIScale")
		if not self.hitMarkerUIScale then
			self.hitMarkerUIScale = Instance.new("UIScale")
			self.hitMarkerUIScale.Parent = hitMarkerFrame
		end
		self.hitMarkerUIScale.Scale = 1
...
function WeaponsGui:onScreenSizeChanged()
	...
	-- Crosshair piece sizing is now handled by UIScale on the crosshairFrame
end
...
function WeaponsGui:OnHitOtherPlayer(damage, humanoidHit) -- show hit indicator, then fade
	...
	TweenService:Create(self.hitMarker, fadeInfo, { ImageTransparency = 1 }):Play()

	-- UIScale pop: punch to 1.3 then settle to 1
	if self.hitMarkerUIScale then
		self.hitMarkerUIScale.Scale = 1.3
		TweenService:Create(self.hitMarkerUIScale, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1 }):Play()
	end

	DamageBillboardHandler:ShowDamageBillboard(damage, humanoidHit.Parent.PrimaryPart)
end
...
function WeaponsGui:onRenderStepped(dt)
	...
	if self.crosshairUIScale and self.crosshairEnabled then
		self.crosshairUIScale.Scale = self.crosshairScale * self.crosshairWeaponScale
	end
end
